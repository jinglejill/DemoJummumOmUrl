<?php
    ...
    $sql = "select * from DEMO_JUMMUM_OM.UserAccount where username = '$username' and password = '$password'";
    $selectedRow = getSelectedRow($sql);
    if(sizeof($selectedRow)==0)
    {
        //download UserAccount
        $sql = "select * from DEMO_JUMMUM_OM.UserAccount where 0";
        writeToLog("sql = " . $sql);
    ...
    else
    {
        ...
        $sql = "select * from DEMO_JUMMUM_OM.Branch where BranchID = '$branchID';";
        $selectedRow = getSelectedRow($sql);
...
//        putAlertToDevice();
    ...
    //device-----***********************
    //get last DbName
    $sql = "select * from DEMO_JUMMUM_OM.`device` where DeviceToken = '$deviceToken'";
    $selectedRow = getSelectedRow($sql);
    if(sizeof($selectedRow)>0)
    {
        ...
        if($dbName != $lastDb)
        {
            ...
            $sql = "delete from DEMO_JUMMUM_OM.`device` where DeviceToken = '$deviceToken'";
            $ret = doQueryTask($sql);
            if($ret != "")
            {
                mysqli_rollback($con);
            ...
            //OM query statement
            $sql = "insert into DEMO_JUMMUM_OM.`device` (`DbName`,`DeviceToken`) values('$dbName','$deviceToken')";
            $ret = doQueryTask($sql);
            if($ret != "")
            {
                mysqli_rollback($con);
    ...
    $sql = "select * from $dbName.setting union select SettingID+1000, `KeyName`, `Value`,Type, Remark, `ModifiedUser`, `ModifiedDate` from DEMO_JUMMUM_OM.setting where type = 2;";
